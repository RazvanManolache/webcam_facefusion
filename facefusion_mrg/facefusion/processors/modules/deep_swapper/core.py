...
def create_static_model_set(download_scope : DownloadScope) -> ModelSet:
	...
	# Custom installs also carry a small ``.hash`` sidecar.  Both files share
	# the same stem, so registering every file makes the later hash entry
	# overwrite the actual DFM source and ONNX Runtime tries to parse eight
	# bytes of checksum text as a model.  Only DFM files are inference models.
	custom_model_file_paths = [
		model_file_path
		for model_file_path in resolve_file_paths(resolve_relative_path('../.assets/models/custom'))
...
def swap_face(target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
	...
	deep_swapper_morph = create_deep_swapper_morph(int(state_manager.get_item('deep_swapper_morph') or 0))
...
def create_deep_swapper_morph(deep_swapper_morph : int) -> DeepSwapperMorph:
	value = numpy.interp(deep_swapper_morph, [ 0, 100 ], [ 0, 1 ])
...
def prepare_crop_frame(crop_vision_frame : VisionFrame) -> VisionFrame:
	...
	crop_vision_frame = crop_vision_frame.astype(numpy.float32)
	numpy.multiply(crop_vision_frame, numpy.float32(1.0 / 255.0), out = crop_vision_frame)
	crop_vision_frame = numpy.expand_dims(crop_vision_frame, axis = 0)
	return crop_vision_frame


def normalize_crop_frame(crop_vision_frame : VisionFrame) -> VisionFrame:
	numpy.multiply(crop_vision_frame, numpy.float32(255.0), out = crop_vision_frame)
	numpy.clip(crop_vision_frame, 0, 255, out = crop_vision_frame)
