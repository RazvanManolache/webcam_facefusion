...
def get_inference_pool(model_names : Optional[List[str]] = None) -> InferencePool:
	if model_names is None:
		model_names = [ state_manager.get_item('face_occluder_model'), state_manager.get_item('face_parser_model') ]
	_, model_source_set = collect_model_downloads()
	model_source_set = { model_name: model_source_set.get(model_name) for model_name in model_names if model_source_set.get(model_name) }
...
def clear_inference_pool() -> None:
	...
	occluder_model_names = [ 'xseg_1', 'xseg_2', 'xseg_3' ] if face_occluder_model == 'many' else [ face_occluder_model ]
	model_names = occluder_model_names + [ state_manager.get_item('face_parser_model') ]

	for model_name in model_names:
		if model_name:
			inference_manager.clear_inference_pool(__name__, [ model_name ])
	# Also clear the combined context created by older releases.
...
def create_static_box_mask(crop_size : Tuple[int, int], face_mask_blur : float, face_mask_padding : Tuple[float, float, float, float]) -> Mask:
...
def create_box_mask(crop_vision_frame : VisionFrame, face_mask_blur : float, face_mask_padding : Padding) -> Mask:
	crop_size = tuple(crop_vision_frame.shape[:2][::-1])
	padding = tuple(float(value) for value in face_mask_padding)
	return create_static_box_mask(crop_size, float(face_mask_blur), padding)
...
def forward_occlude_face(prepare_vision_frame : VisionFrame, model_name : str) -> Mask:
	face_occluder = get_inference_pool([ model_name ]).get(model_name)
...
def forward_parse_face(prepare_vision_frame : VisionFrame) -> Mask:
	model_name = state_manager.get_item('face_parser_model')
	face_parser = get_inference_pool([ model_name ]).get(model_name)
